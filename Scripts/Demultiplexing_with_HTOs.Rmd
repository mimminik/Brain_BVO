...
rep1.umi <- Read10X("/links/groups/treutlein/DATA/sequencing/20220802_P2044_AKANKSHA_FATIMA_MARINA_HSIUCHUAN/processed/rep1/outs/filtered_feature_bc_matrix/")
rep2.umi <- Read10X("/links/groups/treutlein/DATA/sequencing/20220802_P2044_AKANKSHA_FATIMA_MARINA_HSIUCHUAN/processed/rep2/outs/filtered_feature_bc_matrix/")

# Save the mRNA count natrices as rds files
saveRDS(rep1.umi, "/home/marinani/PhD_Projects/Vascular_Organoids/Analysis/Brain_BVOs/Objects/rep1_umi_mtx.rds")
saveRDS(rep2.umi, "/home/marinani/PhD_Projects/Vascular_Organoids/Analysis/Brain_BVOs/Objects/rep2_umi_mtx.rds")

# Load the HTO count matrices
rep1.hto <- readRDS("/home/marinani/PhD_Projects/Vascular_Organoids/Analysis/Brain_BVOs/Objects/rep1_hto_mtx.rds")
rep2.hto <- readRDS("/home/marinani/PhD_Projects/Vascular_Organoids/Analysis/Brain_BVOs/Objects/rep2_hto_mtx.rds")
...
# Important about normalization
# Normalization of the data leads to vastly different results, I tested this.
# For CLR (centered log ratio), one can choose margin = 1 (normalize across features) or margin = 2 (normalize across cells). 
# According to people from Satija lab, in some cases, cell-based normalization fails. 
# This is because cell-normalization makes an assumption that the total ADT/HTO counts should be constant across cells. 
# That can become a significant issue if you have cell populations in your data, but did not add protein markers for them (this is also an issue for scRNA-seq, but is significantly mitigated because at least you measure many genes).
# However, gene-based normalization can fail when there is significant heterogeneity in sequencing depth, or cell size. The optimal strategy depends on the AB panel, and heterogeneity of the sample.
# It is wise to try both across features and across genes, as I did here. Unfortunately, there is no one clear and correct answer...


# My results were:

# with margin 1 (normalize across features) 
# replicate 1
...
# with margin 2 (normalize across cells)
# replicate 1
#Doublet Negative  Singlet 
#    4277       35    12604 
...
table(rep1.hashtag$hash.ID)
table(rep2.hashtag$hash.ID)

```

```{r save hashed objects}

saveRDS(rep1.hashtag, "Objects/seurat.rep1.hashtag.rds")
saveRDS(rep2.hashtag, "Objects/seurat.rep2.hashtag.rds")
...
## Save the hashed object containing only singlets
```{r save hashed singlet objects}

saveRDS(rep1.singlet, "Objects/seurat.rep1.hashtag.singlet.rds")
saveRDS(rep2.singlet, "Objects/seurat.rep2.hashtag.singlet.rds")
